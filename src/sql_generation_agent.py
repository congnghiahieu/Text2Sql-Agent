...
class SQLQueryGenerator(BaseModel):
    query: str = Field("ONLY GENERATE THE SQL QUERY IN 'query' JSON KEY. NEVER EVER PROVIDE ADDITIONAL TEXT OR INFORMATION.")


# SQL generation function with enhanced prompt template for SQLite
def generate_sql_query(user_question: str, table_info: str, chat_history: list, dialect: str = 'sqlite') -> str:
