...
st.subheader(
    "Interact with a powerful SQL-driven assistant to query and explore data seamlessly!"
)
...
if prompt := st.chat_input("Ask a question about the data:"):
    ...
    plot_flag = any(
        keyword in prompt.lower() for keyword in ["plot", "chart", "bar", "visualize"]
    )
    ...
    st.session_state.chat_history.append(
        {"role": "assistant", "content": markdown_table}
    )
    st.session_state.chat_history.append(
        {"role": "assistant", "content": f"Here is the Data Summary:\n{summary}"}
    )
